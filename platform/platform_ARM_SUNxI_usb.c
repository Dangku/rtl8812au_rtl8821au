...
extern void sunxi_wlan_usb_set_power(int on);
extern int sunxi_wlan_usb_get_bus_index(void);
static int usb_busnum = -1;
#endif
...
int platform_wifi_power_on(void)
{
	...
	{
		usb_busnum = sunxi_wlan_usb_get_bus_index();
		if (usb_busnum < 0 && usb_busnum > 2)
		{
			printk("unsupported wlan_usb_busnum (%u)\n", usb_busnum);
			ret = -ENOMEM;
			goto exit;
		}
		...
		sunxi_wlan_usb_set_power(1);
		mdelay(10);

#if !(defined(CONFIG_RTL8723A)) && !(defined(CONFIG_RTL8723B))
		//sunxi_usb_enable_hcd(usb_busnum);
#endif
	}
...
#if defined(CONFIG_PLATFORM_ARM_SUN8I)
	#if !(defined(CONFIG_RTL8723A)) && !(defined(CONFIG_RTL8723B))
	//sunxi_usb_disable_hcd(usb_busnum);
	#endif
	sunxi_wlan_usb_set_power(0);
